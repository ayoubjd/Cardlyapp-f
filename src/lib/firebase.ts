...
export async function addDeckToFirestore(userId: string, deck: any) {
  const { id, ...rest } = deck;
  return addDoc(collection(firestore, "users", userId, "decks"), deckToFirestore(rest));
}
...
export async function addFlashcardToFirestore(userId: string, card: any) {
  ...
  return addDoc(collection(firestore, "users", userId, "flashcards"), flashcardToFirestore(rest));
}
...
export async function addSessionToFirestore(userId: string, session: any) {
  ...
  return addDoc(collection(firestore, "users", userId, "studySessions"), sessionToFirestore(rest));
}
